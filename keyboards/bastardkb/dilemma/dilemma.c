...
static bool     g_last_sniping_state      = false;
#    endif     // DILEMMA_AUTO_SNIPING_ON_LAYER

report_mouse_t pointing_device_task_kb(report_mouse_t mouse_report) {
    if (is_keyboard_master()) {
#    ifdef DILEMMA_AUTO_SNIPING_ON_LAYER
        // Track sniping state changes directly in the pointing device task
        bool current_sniping_state = g_dilemma_config.is_sniping_enabled;
        if (current_sniping_state != g_last_sniping_state) {
            // Sniping state changed - reset spike detection
            g_frames_since_cpi_change = 0;
            g_spike_found = false;
            g_last_sniping_state = current_sniping_state;
        }
